...
# Полностью скрываем все стандартные элементы Streamlit
...
def load_all_data(city: str, rooms: tuple) -> Optional[pd.DataFrame]:
    """
    Загружает данные с ЦИАН. Результат кэшируется на 1 час.
    Параметр rooms передаётся как кортеж для хэширования.
    """
    ...
    for i in range(page, max_pages + 1):
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                status_text.info("✅ Все доступные страницы обработаны!")
                break
        except Exception:
